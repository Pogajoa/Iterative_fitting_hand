...
def fit_single_frame(img,
    ...
    if save_meshes or visualize:
        ...
        # import hand idx correspond
       
        
        corr_fname = '/smplify-x/correspondences/MANO_SMPLX_vertex_ids.pkl'
        hand = 'both'
        ...
        test = []
        for k in hand_idxs:
            idx = list(out_mesh.faces_sparse.getrow(k).indices)
            test += idx
        
        new = np.array(test)

        hand_mesh = out_mesh.submesh([new], only_watertight=False, append=True )
        

        #hand_mesh.show()
        
        cloud_original = trimesh.points.PointCloud(vertices[hand_idxs])
        ...
        out_mesh = copy.deepcopy(hand_mesh)
    ...
    if visualize:
        ...
        #color = color.astype(np.float32) / 30.0
